...
- The `body` key defines which single top-level field in the request will be
  ...
  - Fields **should not** use the `json_name` annotation to alter the field
    name in JSON, unless doing so for compatibility reasons.
...
- **2021-05-06**: Added clarification in fields.
- **2019-09-23**: Added a statement about guiding
  encouragment `json_name`.
